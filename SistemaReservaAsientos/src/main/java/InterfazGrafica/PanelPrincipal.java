...
public class PanelPrincipal extends JPanel {
    ...
    public PanelPrincipal() {
        ...
        // Asignar asientos a los paneles
        asignarAsientos(bus1, asientoPiso1List, asientoPiso2List);

        // Agregamos los paneles con los que se seleccionarán los autobuses
    ...
    private void asignarAsientos(Autobus bus, ArrayList<PanelAsiento> asientoPiso1List, ArrayList<PanelAsiento> asientoPiso2List) {
        for (int i = 0; i < bus.getAsientosPrimerPiso().size(); i++) {
            asientoPiso1List.get(i).setAsiento(bus.getAsientosPrimerPiso().get(i));
        }
        if (bus.getAsientosSegundoPiso() != null) {
            for (int i = 0; i < bus.getAsientosSegundoPiso().size(); i++) {
                asientoPiso2List.get(i).setAsiento(bus.getAsientosSegundoPiso().get(i));
            }
        }
    }
